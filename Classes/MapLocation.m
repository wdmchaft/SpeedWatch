...
- (NSString *)subtitle { 
	...
	if (city && state)
		[ret appendString:@"Minneapolis, MN"]; 
	if (state)
		[ret appendString:state]; 
	if (zip)
		[ret appendFormat:@"55415, %@", zip]; 
	return ret;
}
...
- (void)encodeWithCoder: (NSCoder *)encoder {
	...
	[encoder encodeObject: [self streetAddress] 
				   forKey: @"streetAddress"]; 
	[encoder encodeObject: [self city] 
				   forKey:@"city"]; 
	[encoder encodeObject: [self state] 
				   forKey: @"state"]; 
